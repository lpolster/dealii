...
void Step6<dim>::setup_system ()
{
    ...
    VectorTools::interpolate_boundary_values (dof_handler_adaptiveIntegration,
                                              ...
                                              ZeroFunction<dim>(),
                                              constraints);
    //add_constraints_on_interface(dof_handler_adaptiveIntegration, fe, constraints);
    constraints.close ();
        ...
        for (unsigned int q_index=0; q_index<n_q_points; ++q_index){      // loop over all quadrature points
            ...
            for (unsigned int i=0; i<dofs_per_cell; ++i)  {                   // loop over degrees of freedom
                for (unsigned int j=0; j<dofs_per_cell; ++j)  {              // loop over degrees of freedom
                    ...
                    cell_matrix(i,j) += (coefficient_values[q_index] * fe_values.shape_grad(i,q_index) * //coefficient_values[q_index] *
                                         fe_values.shape_grad(j,q_index) *
                                         fe_values.JxW(q_index)); //* fe_values_solutionGrid.JxW(q_index)
//                                                std::cout<<fe_values.get_quadrature_points()[q_index][0]<<std::endl;
                    cell_matrix(i,j) += beta * fe_values.shape_value(i,q_index) *
...
//    ofs_cell_matrix<<"cell: "<<cell<<std::endl;
//    cell_matrix.print(ofs_cell_matrix);
//    ofs_cell_matrix<<"----------------------------------------------"<<std::endl;
//    ofs_cell_matrix.close();

    cell-> get_dof_indices (local_dof_indices);              // return the global indices of the degrees of freedom located on this object

...
void adaptiveNumberOfQuadraturePoints<dim>::setup_system ()
{
    ...
    solution.reinit (dof_handler_adaptiveIntegration.n_dofs());
    system_rhs.reinit (dof_handler_adaptiveIntegration.n_dofs());

    constraints.clear ();
    DoFTools::make_hanging_node_constraints (dof_handler_adaptiveIntegration,
                                             constraints);
    VectorTools::interpolate_boundary_values (dof_handler_adaptiveIntegration,
                                              0,
                                              ZeroFunction<dim>(),
                                              constraints);
    constraints.close ();

    CompressedSparsityPattern c_sparsity(dof_handler_adaptiveIntegration.n_dofs());
    DoFTools::make_sparsity_pattern(dof_handler_adaptiveIntegration,
                                    c_sparsity,
                                    constraints,
    ...
    if (cell_is_cut_by_boundary(solution_cell))
    {
        for(; cell!=endc; ++cell)
            ...
            if (cell->vertex(0)[0] >= solution_cell->vertex(0)[0] && cell->vertex(0)[1] >= solution_cell->vertex(0)[1]  && cell->vertex(3)[0] <= solution_cell->vertex(3)[0] && cell->vertex(3)[1] <= solution_cell->vertex(3)[1] )
            {
                ...
                q_weights.insert(q_weights.end(),fe_values_temp.get_quadrature().get_weights().begin(),
...
void adaptiveNumberOfQuadraturePoints<dim>::assemble_system (std::function<double(double, double)> f)
{
    ...
    FEValues<dim> fe_values_adaptiveIntegration (fe_adaptiveIntegration, quadrature_formula, update_quadrature_points |  update_gradients | update_JxW_values | update_jacobians  |  update_values);
    
    const unsigned int   dofs_per_cell = fe_adaptiveIntegration.dofs_per_cell;
    ...
    {
        // std::cout<<"Collecting quadrature points of cell "<<solution_cell<<std::endl;
        ...
        for (unsigned int i = 0; i< collected_quadrature.size(); ++i)
    ...
    for (; cell!=endc; ++cell)                                            // loop over all active cells
    {
        cell_matrix = 0;
        cell_rhs = 0;
        fe_values_adaptiveIntegration.reinit(cell);

        value_list (fe_values_adaptiveIntegration.get_quadrature_points(),
        ...
        for (unsigned int q_index=0; q_index<n_q_points; ++q_index){      // loop over all quadrature points
            ...
            for (unsigned int i=0; i<dofs_per_cell; ++i)  {                   // loop over degrees of freedom
                for (unsigned int j=0; j<dofs_per_cell; ++j)  {              // loop over degrees of freedom
                    ...
                    cell_matrix(i,j) += (coefficient_values[q_index] * fe_values_adaptiveIntegration.shape_grad(i,q_index) * //
                                         fe_values_adaptiveIntegration.shape_grad(j,q_index) *
                                         fe_values_adaptiveIntegration.JxW(q_index));
                    if (cell_is_cut_by_boundary(cell))
                    cell_matrix(i,j) += beta * fe_values_adaptiveIntegration.shape_value(i,q_index) *
                                        fe_values_adaptiveIntegration.shape_value(j,q_index);
                }
                cell_rhs(i) += (coefficient_values[q_index] * fe_values_adaptiveIntegration.shape_value(i,q_index) * // the cell rhs
                                1.0 *
                                fe_values_adaptiveIntegration.JxW(q_index));
                if (cell_is_cut_by_boundary(cell))
                cell_rhs(i) += beta * fe_values_adaptiveIntegration.shape_value(i,q_index) * 0.0; // Dirichlet boundary condition zero
            }

        }

    cell-> get_dof_indices (local_dof_indices);  // return the global indices of the dof located on this object
...
void adaptiveNumberOfQuadraturePoints<dim>::output_results ()
{
    ...
    DataOut<2> data_out;
    data_out.attach_dof_handler (dof_handler_adaptiveIntegration);

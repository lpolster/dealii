...
namespace FCMImplementation{ // use namespace to avoid the problems that result if names of different functions or variables collide
    ...
    private:
        ...
        double Nitsche_matrix_terms(int q_index, int i, int j, FEValues<2> &fe_values_on_boundary_segment,  myPolygon::segment my_segment);
        double Nitsche_rhs_terms(int q_index, int i, FEValues<2> &fe_values_on_boundary_segment,  myPolygon::segment my_segment);
                ...
                for (unsigned int k = 0; k < segment_indices.size(); ++ k){
                    ...
                    for (unsigned int q_index=0; q_index<my_segment.q_points.size(); ++q_index)
                    {
                        for (unsigned int i=0; i<dofs_per_cell; ++i)  { // loop over degrees of freedom
                            ...
                            cell_rhs(i) += Nitsche_rhs_terms(q_index, i, fe_values_on_boundary_segment, my_segment);
    ...
    double Step3::Nitsche_matrix_terms(int q_index, int i, int j, FEValues<2> &fe_values_on_boundary_segment,  myPolygon::segment my_segment)
    {
        double Nitsche_matrix_terms = 0.0;
        Nitsche_matrix_terms += beta_h * (fe_values_on_boundary_segment.shape_value(i,q_index) *
    ...
    double Step3::Nitsche_rhs_terms(int q_index, int i, FEValues<2> &fe_values_on_boundary_segment,  myPolygon::segment my_segment)
    {
        ...
        Nitsche_rhs_terms -= (dirichlet_boundary_value * fe_values_on_boundary_segment.shape_grad(i,q_index) *
                              ...
                              fe_values_on_boundary_segment.get_quadrature().get_weights()[q_index]);
        Nitsche_rhs_terms +=  (beta_h * fe_values_on_boundary_segment.shape_value(i,q_index) *
